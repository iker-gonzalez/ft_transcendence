  ...
  async getMessageInRoom(roomName: string): Promise<any> {
    ...
    for (const message of conversationsChannel) {
      conversationDTO.push(
        new ConversationMessageDTO(message, message.sender, null),
      );
    }
    ...
    allinfo.setIntrasOfMemeber(
      await this.findUserIntraByIArrayd(chatRoom.users),
      'users',
    );
    allinfo.setIntrasOfMemeber(
      await this.findUserIntraByIArrayd(chatRoom.adminUsers),
      'admin',
    );
    allinfo.setIntrasOfMemeber(
      await this.findUserIntraByIArrayd(chatRoom.mutedUsers),
      'muted',
    );
    allinfo.setIntrasOfMemeber(
      await this.findUserIntraByIArrayd(chatRoom.bannedUsers),
      'banned',
    );
  ...
  async muteUserInChannel(
    ...
    try {
      ...
      if (muteUserId == foundChatRoom.ownerId)
        throw new BadRequestException(
          'Cannot MUTE to the owner of the channel',
        );
  ...
  async addPasswordChannel(
    ...
    try {
      if (!channelRoom || !ownerId || !password)
        throw new BadRequestException(
          'channelRoom or ownerIntra or password  are null',
        );
      ...
      await this.prisma.chatRoom.update({
        where: { id: foundChatRoom.id },
        data: { password: password, type: ChannelType.PROTECTED },
      });
    } catch (error) {
      console.error('Error:', error);
    }
