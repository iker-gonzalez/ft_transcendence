...
export class AllChannelInfo {
  ...
  constructor(
    achannelMessage: ConversationMessageDTO[],
    chatRoom: ChatRoom,
    ownerIntra: number,
  ) {
  ...
  setIntrasOfMemeber(memeberIntra: IntraUsernameDTO[], member: string) {
    if (member == 'users') this.usersInfo = memeberIntra;
    else if (member == 'admin') this.adminsInfo = memeberIntra;
    else if (member == 'banned') this.bannedInfo = memeberIntra;
    else if (member == 'muted') this.mutedInfo = memeberIntra;
  }
}
